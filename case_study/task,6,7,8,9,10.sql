...
SELECT distinct dv.ma_dich_vu, ten_dich_vu, dien_tich, chi_phi_thue, ldv.ten_loai_dich_vu
FROM dich_vu as dv
join hop_dong as hd on hd.ma_dich_vu = dv.ma_dich_vu
join loai_dich_vu as ldv on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu
where dv.ma_dich_vu not in (select distinct ma_dich_vu from hop_dong where year(ngay_lam_hop_dong)=2021 and (month(ngay_lam_hop_dong) in(1,2,3)));
...
join hop_dong as hd on hd.ma_dich_vu = dv.ma_dich_vu
join loai_dich_vu as ldv on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu
where year(ngay_lam_hop_dong)=2020 and dv.ma_dich_vu not in (select distinct ma_dich_vu from hop_dong where year(ngay_lam_hop_dong)=2021);
